...
function exerciseDisplayName(e){
 const meta=catalogOptionMeta(e),base=canonicalExerciseName(e?.name||"Übung"),variant=String(e?.variant||"").trim(),source=meta.nameSource||"equipment";
 if(source==="variant"){
  if(!variant)return base;
  return `${variant} ${base}`
 }
 if(source==="none")return base;
 const eq=meta.showEquipment?String(e?.equipmentChoice||meta.defaultEquipment||"").trim():"";if(!eq||eq==="-")return base;
...
function exerciseInlineMeta(e){
 const bits=[],meta=catalogOptionMeta(e),source=meta.nameSource||"equipment";
 const variant=String(e?.variant||"").trim();
...
function exerciseWorkSeconds(e){const sets=Math.max(1,Number(e?.sets)||1);return e?.measureMode==="time"?sets*Math.max(15,Number(e.timeSeconds)||60):sets*60}
function estimateMinutes(p){
 const ex=p?.exercises||[];let sec=0,seen=new Set(),countedExercises=0;
 ex.forEach(e=>{if(groupMethod(e.setTechnique)&&e.techniqueGroup){if(seen.has(e.techniqueGroup))return;seen.add(e.techniqueGroup);const members=ex.filter(x=>x.techniqueGroup===e.techniqueGroup),rounds=Math.max(1,...members.map(x=>Number(x.sets)||1));countedExercises+=members.length;sec+=members.reduce((sum,x)=>sum+(x.measureMode==="time"?rounds*Math.max(15,Number(x.timeSeconds)||60):rounds*60)+45,0);sec+=Math.max(0,rounds-1)*restSeconds(members[0],90);return}const sets=Math.max(1,Number(e.sets)||1);countedExercises++;sec+=45+exerciseWorkSeconds(e)+Math.max(0,sets-1)*restSeconds(e,90)});
 sec+=Math.max(0,countedExercises-1)*30; return Math.max(1,Math.ceil(sec/60))
}
...
function methodRepConfigMarkup(e,prefix){
 if(e.setTechnique==="cluster"){e.methodData=e.methodData||{};const total=(Number(e.methodData.blocks)||4)*(Number(e.methodData.clusterReps)||2);e.reps=String(total);return`<div class="cluster-derived-total">${total} WDH. gesamt · ergibt sich aus Cluster × WDH. pro Cluster</div>`}
...
function savePlanAddFormToDraft(){
 ...
 if($("paDropPct"))e.methodData.dropPercent=Number($("paDropPct").value)||20;if($("paClusterBlocks"))e.methodData.blocks=Number($("paClusterBlocks").value)||4;if($("paClusterReps"))e.methodData.clusterReps=Number($("paClusterReps").value)||2;if(e.setTechnique==="cluster")e.reps=String((Number(e.methodData.blocks)||4)*(Number(e.methodData.clusterReps)||2));if($("paIntraRest"))e.methodData.intraRest=Number($("paIntraRest").value)||20;saveMethodRepConfig(e,"pa")
}
...
function renderCompactPartnerConfig(){
 ...
 renderSheetState({
  ...
  <div class="mode-switch"><button type="button" class="chip ${e.measureMode!=="time"?"active":""}" id="partnerModeReps">Wiederholungen</button><button type="button" class="chip ${e.measureMode==="time"?"active":""}" id="partnerModeTime" ${methodAllowsTime(method)?"":"disabled"}>Zeit</button></div>
 ...
 $("partnerModeReps").onclick=()=>{e.measureMode="reps";renderCompactPartnerConfig()};$("partnerModeTime").onclick=()=>{if(!methodAllowsTime(method))return;e.measureMode="time";e.timeSeconds=Math.max(15,Number(e.timeSeconds)||60);renderCompactPartnerConfig()};
...
function configureExercise(i,flow=null){
 ...
 const bind=()=>{
  ...
  $("cfgSave").onclick=()=>{
    ...
    if($("cfgGiantCount"))draft.methodData.giantCount=Number($("cfgGiantCount").value)||3;if($("cfgClusterBlocks"))draft.methodData.blocks=Number($("cfgClusterBlocks").value)||4;if($("cfgClusterReps"))draft.methodData.clusterReps=Number($("cfgClusterReps").value)||2;if(draft.setTechnique==="cluster")draft.reps=String((Number(draft.methodData.blocks)||4)*(Number(draft.methodData.clusterReps)||2));if($("cfgIntraRest"))draft.methodData.intraRest=Number($("cfgIntraRest").value)||20;
...
function openPreview(p){
 ...
 $("previewBody").innerHTML=`<div class="preview-live-shell preview-static-v43">${previewVisualGroups(pp.exercises).map(previewMethodCard).join("")}</div>`;
...
function renderSets(e,ei){
 ...
 if(e.measureMode==="time"){
  return`<div class="set-head unified-time-head"><span>SATZ</span><span>ZEIT</span><span>LEISTUNG</span><span></span><span></span></div>${e.liveSets.map((s,si)=>`<div class="set-row unified-time-row"><span>${si+1}</span><div class="time-input-shell unified-time-shell"><input type="text" inputmode="none" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" class="${s.completed?"rated-time-value":""}" data-time-field="1" data-input="${ei}|${si}|time" placeholder="${liveTimeBoxPlaceholder(s)}" value="${liveTimeBoxValue(s)}"><button type="button" class="time-play time-play-inside" data-time-play="${ei}|${si}" aria-label="Timer starten">▶</button></div><input class="unified-performance-input" type="text" autocomplete="off" autocorrect="off" data-input="${ei}|${si}|level" placeholder="${esc(s._suggested?.level||"Leistung")}" value="${esc(s.level||"")}"><button class="set-check ${s.completed?"done":""} ${ratingClass(s)} ${canRateSet(e,s)?"ready":""}" data-check="${ei}|${si}">✓</button><button class="remove-mini" data-remove-live-set="${ei}|${si}">−</button></div>${ratingMarkup(ei,si,s)}`).join("")}`
 }
 if(e.setTechnique==="cluster"){
  return e.liveSets.map((s,si)=>{
 ...
 if(e.setTechnique==="dropset"){
  return e.liveSets.map((s,si)=>{
   ...
   return`<div class="advanced-head"><span>SATZ ${si+1}</span><span></span><span>KG</span><span>WDH.</span><span></span><span></span></div>${segs.map(([g,gi])=>`<div class="advanced-row"><span>${gi+1}</span><span class="small">${esc(g.label)}</span><input type="text" inputmode="decimal" autocomplete="off" autocorrect="off" data-input="${ei}|${si}|sw|${gi}" placeholder="${esc(g._suggested?.weight||"KG")}" value="${esc(g.weight)}"><input type="text" inputmode="decimal" autocomplete="off" autocorrect="off" data-input="${ei}|${si}|sr|${gi}" placeholder="${esc(g._suggested?.reps||"WDH.")}" value="${esc(g.reps)}"><span></span><span></span></div>${gi<segs.length-1?`<div class="drop-reduction-divider"><span></span><b>−${Number(e.methodData?.dropPercent)||20}% Gewicht</b><span></span></div>`:""}`).join("")}<div class="space" style="margin-top:6px"><button class="set-check ${s.completed?"done":""} ${ratingClass(s)} ${canRateSet(e,s)?"ready":""}" data-check="${ei}|${si}">✓</button><button class="remove-mini" data-remove-live-set="${ei}|${si}">−</button></div>${ratingMarkup(ei,si,s)}`}).join("")
 }
...
function finishAndSaveWorkout(){
 if(!activeWorkout)return;
 if(activeWorkout.startedFromUnsavedPlan || !plans.some(p=>String(p.id)===String(activeWorkout.planId||activeWorkout.sourcePlanId||""))){
