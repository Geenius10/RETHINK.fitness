...
function openExerciseDetail(name,{onAdd=null}={}){
 const e=findExercise(name);currentExercise=e;$("exerciseDetailTitle").textContent=exerciseDisplayName(e);
...
function renderPartnerExerciseDetail(name){
 ...
 renderSheetState({title:name,scroll:0,body:`<div class="detail-section"><div class="small">TRAININGSART</div><strong>${esc(e.category||e.categories?.[0]||"—")}</strong></div><div class="detail-section"><div class="small">BEREICHE</div><strong>${esc((e.muscles||[]).join(", ")||"—")}</strong></div>${e.custom?"":`<div class="detail-copy"><h2>Ausführung</h2><p>${esc(e.execution||"")}</p></div>`}<button id="partnerDetailAdd" class="primary" style="width:100%">Als Übung ${String.fromCharCode(65+f.drafts.length)} wählen</button>`,onBack:renderPartnerExercisePicker,onClose:cancelPlanAddFlow});
...
function combinedMemberControls(x,si,gi,showLabels=true){
 ...
 if(x.e.measureMode==="time"){
  const action=s.completed
   ...
   :`<button type="button" class="set-check time-rating-action ready" data-time-play="${x.i}|${si}" aria-label="Timer starten und anschließend bewerten">▶</button>`;
...
function toggleTimeTimer(key,btn){
 ...
 if(timeSetTimers.has(id)){
   ...
   setTimeout(()=>openSetRating(ei,si),120);return
 }
...
function structuralExercises(exercises){
 return (exercises||[]).map(x=>{const e=cleanPlanExerciseForSave(x||{});return {
  name:e.name||"",variant:e.variant||"",equipment:e.equipment||"",measureMode:e.measureMode||"reps",perSide:!!e.perSide,
